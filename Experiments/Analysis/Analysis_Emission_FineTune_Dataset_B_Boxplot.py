...
def plot_emission_finetune_side_by_side(df, dataset):
    ...
    g.fig.suptitle(f"Emission with/without Fine-Tuning - Dataset B")

    # Adjust layout
    plt.tight_layout()

    # Save
    png_path = os.path.join(OUTPUT_DIR, f"Emission_FineTune_dataset_B_SideBySide.png")
    pdf_path = os.path.join(OUTPUT_DIR, f"Emission_FineTune_dataset_B_SideBySide.pdf")
