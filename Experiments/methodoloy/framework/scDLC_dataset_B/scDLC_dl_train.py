...
# Update output directories to use dataset_B
...
for method, params in methods.items():
    ...
    config_path = f"../../models/dataset_B/scDLC_dataset_B_{method}_config.json"
    with open(config_path, "w") as f:
        json.dump(architecture_config, f)

    # Initialize the model; note that input_size is 2000 (number of features)
    ...
    # Save the pre-training model with updated naming (dataset_B)
    torch.save(model.state_dict(), f"../../models/dataset_B/scDLC_DL_dataset_B_{method}_pre_train.pth")

per_epoch_results_df = pd.DataFrame(per_epoch_results, columns=['Method', 'Epoch', 'Train_Accuracy', 'Test_Accuracy'])
per_epoch_results_df.to_csv("../../results/dataset_B/scDLC_DL_per_epoch_result_pre_train.csv", index=False)
...
results_df.to_csv("../../results/dataset_B/scDLC_DL_results_pre_train.csv", index=False)
