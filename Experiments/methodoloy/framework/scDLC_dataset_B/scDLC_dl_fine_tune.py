...
# Define folder paths for Dataset B
...
for client, file in clients.items():
    ...
    for method, params in methods.items():
        ...
        torch.save(model.state_dict(), f"../../models/dataset_B/scDLC_DL_dataset_B_{method}_without_fine_tune_{client}.pth")
        
        print(f"Training Method: {method}, Client: {client}, With Fine-Tuning")
        tracker = EmissionsTracker()
        tracker.start()
        
        # Load the pre-trained model weights (assumed to be saved earlier from training on dataset_B)
        model.load_state_dict(torch.load(f"../../models/dataset_B/scDLC_DL_dataset_B_{method}_pre_train.pth"))
        model = model.to(device)
        ...
        torch.save(model.state_dict(), f"../../models/dataset_B/scDLC_DL_dataset_B_{method}_with_fine_tune_{client}.pth")
    ...
    df_results.to_csv(f"../../results/dataset_B/scDLC_DL_results_fine_tune_{client}.csv", index=False)
    
    
df_emissions = pd.DataFrame(emission_results, columns=["Client", "Method", "FineTune", "Emission (kWh)"])
df_emissions.to_csv(f"../../results/dataset_B/scDLC_DL_Client_A_B_emission.csv", index=False)
