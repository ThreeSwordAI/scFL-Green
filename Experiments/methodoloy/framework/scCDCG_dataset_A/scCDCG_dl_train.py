...
# Define paths and load data (example with dataset_A as before)
folder_path = "../../processed_data/dataset_A"
chunks_to_train = [f"{folder_path}/dataset_A_chunk_{i + 1}.h5ad" for i in range(8)]
...
# Different training configurations (methods)
methods = {
...
for method, params in methods.items():
    ...
    config_path = f"../../models/dataset_A/scCDCG_dataset_A_{method}_config.json"
    ...
    for epoch in range(15):
        ...
        with tqdm(total=len(train_loader), desc=f"Epoch {epoch + 1}/15", unit="batch") as pbar:
            for inputs, labels in train_loader:
                ...
                optimizer.zero_grad()
                outputs = model(inputs)  # For simplicity, we do not pass an explicit adjacency.
    ...
    torch.save(model.state_dict(), f"../../models/dataset_A/scCDCG_DL_dataset_A_{method}_pre_train.pth")

per_epoch_results_df = pd.DataFrame(per_epoch_results, columns=['Method', 'Epoch', 'Train_Accuracy', 'Test_Accuracy'])
per_epoch_results_df.to_csv("../../results/dataset_A/scCDCG_DL_per_epoch_result_pre_train.csv", index=False)
...
results_df.to_csv("../../results/dataset_A/scCDCG_DL_results_pre_train.csv", index=False)
