...
# Define file paths for dataset_C (CSV format)
folder_path = "../../processed_data/dataset_C"
# Use chunks 1-4 as client files
...
input_size = first_df.iloc[:, :2000].shape[1]  # expected to be 2000
num_classes = len(first_df.iloc[:, 2000].astype('category').cat.categories)

# Define method-specific parameters for ACTINN federated training
methods = {
    'Baseline': {},
...
for method, params in methods.items():
    ...
    config_path = f"../../models/dataset_C/ACTINN_FL_dataset_C_{method}_config.json"
    ...
    # Create DataLoaders for each client using its own CSV file
    client_loaders = []
    ...
    for rnd in range(global_rounds):
        ...
        if test_accuracy >= 0.90:
            print(f"Early stopping for method {method} at global round {rnd+1} with Acc: {test_accuracy:.4f}")
            break
    ...
    torch.save(global_model.state_dict(), f"../../models/dataset_C/ACTINN_FL_dataset_C_{method}_pre_train.pth")

per_round_results_df = pd.DataFrame(all_round_results, columns=['Method', 'Global_Round', 'Test_Accuracy', 'Test_Loss'])
per_round_results_df.to_csv("../../results/dataset_C/ACTINN_FL_per_round_result.csv", index=False)
...
results_df.to_csv("../../results/dataset_C/ACTINN_FL_results.csv", index=False)

print("Federated Learning pre-training for ACTINN complete for dataset_C. Results saved to CSV.")
