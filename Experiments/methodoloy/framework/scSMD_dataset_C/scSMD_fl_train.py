...
# Define file paths for dataset_C (CSV files)
folder_path = "../../processed_data/dataset_C"
# Use chunks 1-4 for client training (CSV files)
...
for method, params in methods.items():
    ...
    config_path = f"../../models/dataset_C/scSMD_FL_dataset_C_{method}_config.json"
    with open(config_path, "w") as f:
        json.dump(architecture_config, f)

    # Initialize the global scSMD model
    ...
    # Create DataLoaders for each client using its CSV file
    client_loaders = []
    ...
    model_save_path = f"../../models/dataset_C/scSMD_FL_dataset_C_{method}_pre_train.pth"
    torch.save(global_model.state_dict(), model_save_path)
per_round_results_df = pd.DataFrame(all_round_results, columns=['Method', 'Global_Round', 'Test_Accuracy', 'Test_Loss'])
per_round_results_df.to_csv("../../results/dataset_C/scSMD_FL_per_round_result_pre_train.csv", index=False)
...
results_df.to_csv("../../results/dataset_C/scSMD_FL_results_pre_train.csv", index=False)

print("Federated training complete for scSMD on dataset_C. Results saved to CSV.")
