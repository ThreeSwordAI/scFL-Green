...
# Define folder paths for Dataset C (CSV files)
...
for client, file in clients.items():
    ...
    for method, params in methods.items():
        ...
        torch.save(model.state_dict(), f"../../models/dataset_C/scSMD_DL_dataset_C_{method}_without_fine_tune_{client}.pth")
        ...
        model.load_state_dict(torch.load(f"../../models/dataset_C/scSMD_DL_dataset_C_{method}_pre_train.pth"))
        model = model.to(device)
        ...
        torch.save(model.state_dict(), f"../../models/dataset_C/scSMD_DL_dataset_C_{method}_with_fine_tune_{client}.pth")
    ...
    df_results.to_csv(f"../../results/dataset_C/scSMD_DL_results_fine_tune_{client}.csv", index=False)
    
df_emissions = pd.DataFrame(emission_results, columns=["Client", "Method", "FineTune", "Emission (kWh)"])
df_emissions.to_csv("../../results/dataset_C/scSMD_DL_Client_A_B_emission.csv", index=False)

print("Fine-tuning complete for scSMD (DL) on dataset_C. Results saved to CSV.")
