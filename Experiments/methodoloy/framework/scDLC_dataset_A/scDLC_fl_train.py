...
for method, params in methods.items():
    ...
    # Save the architecture configuration (using same settings as before)
    architecture_config = {
        "lstm_size": params.get('lstm_size', 64),
        "num_layers": params.get('num_layers', 2)
    }
    config_path = f"../../models/dataset_A/scDLC_FL_dataset_A_{method}_config.json"
    ...
    # Create DataLoaders for each client using its own chunk.
    client_loaders = []
    ...
    # Define loss criterion (will be used for both local and global evaluation)
    ...
    model_save_path = f"../../models/dataset_A/scDLC_FL_dataset_A_{method}_pre_train.pth"
...
per_round_results_df.to_csv("../../results/dataset_A/scDLC_FL_per_round_result_pre_train.csv", index=False)
...
results_df.to_csv("../../results/dataset_A/scDLC_FL_results_pre_train.csv", index=False)
