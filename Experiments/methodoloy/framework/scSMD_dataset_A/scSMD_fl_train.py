...
client_files = [f"{folder_path}/dataset_A_chunk_{i+1}.h5ad" for i in range(8)]
...
all_round_results = []  # Store per-global-round results
...
for method, params in methods.items():
    ...
    config_path = f"../../models/dataset_A/scSMD_FL_dataset_A_{method}_config.json"
    with open(config_path, "w") as f:
        json.dump(architecture_config, f)

    # Initialize global model
    global_model = scSMD_scRNAseqClassifier(
    ...
    for rnd in range(global_rounds):
        local_weights = []  # Collect local model weights

        # Each client trains locally
        for loader in client_loaders:
            local_model = scSMD_scRNAseqClassifier(
                input_size=input_size,
                num_classes=num_classes,
                latent_dim=architecture_config["latent_dim"]
            )
    ...
    model_save_path = f"../../models/dataset_A/scSMD_FL_dataset_A_{method}_pre_train.pth"
    torch.save(global_model.state_dict(), model_save_path)
per_round_results_df = pd.DataFrame(all_round_results, columns=['Method', 'Global_Round', 'Test_Accuracy', 'Test_Loss'])
per_round_results_df.to_csv("../../results/dataset_A/scSMD_FL_per_round_result_pre_train.csv", index=False)
...
results_df.to_csv("../../results/dataset_A/scSMD_FL_results_pre_train.csv", index=False)

print("Federated training complete for scSMD on dataset_A. Results saved to CSV.")