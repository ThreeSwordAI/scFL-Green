...
# Define file paths for dataset_A (h5ad files)
...
for client, file in clients.items():
    ...
    for method, params in methods.items():
        ...
        torch.save(model.state_dict(), f"../../models/dataset_A/scSMD_DL_dataset_A_{method}_without_fine_tune_{client}.pth")
        ...
        model.load_state_dict(torch.load(f"../../models/dataset_A/scSMD_DL_dataset_A_{method}_pre_train.pth"))
        model = model.to(device)
        ...
        torch.save(model.state_dict(), f"../../models/dataset_A/scSMD_DL_dataset_A_{method}_with_fine_tune_{client}.pth")
    ...
    df_results.to_csv(f"../../results/dataset_A/scSMD_DL_results_fine_tune_{client}.csv", index=False)
    
df_emissions = pd.DataFrame(emission_results, columns=["Client", "Method", "FineTune", "Emission (kWh)"])
df_emissions.to_csv("../../results/dataset_A/scSMD_DL_Client_A_B_emission.csv", index=False)

print("Fine-tuning complete for scSMD on dataset_A. Results saved.")
