...
def process_test_data(file_path):
    # Read CSV file: first 2000 columns are features and the 2001st column is the label.
    ...
    # Return dataset and the categorical labels from the CSV
...
for method in methods.keys():
    ...
    # Model file path for scSMD DL pre-trained model (dataset_B)
    model_path = f"../../models/dataset_B/scSMD_DL_dataset_B_{method}_pre_train.pth"
    ...
    model = scSMD_scRNAseqClassifier(
        input_size=test_data[0][0].shape[0],  # should be 2000
...
df_results.to_csv("../../results/dataset_B/scSMD_DL_test_result_distribution.csv", index=False)
