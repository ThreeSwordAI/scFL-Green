...
for client, client_file in clients.items():
    ...
    for method, params in methods.items():
        ...
        pre_trained_path = f"../../models/dataset_B/scSMD_FL_dataset_B_{method}_pre_train.pth"
        ...
        torch.save(model.state_dict(), f"../../models/dataset_B/scSMD_FL_dataset_B_{method}_with_fine_tune_{client}.pth")
    ...
    df_results.to_csv(f"../../results/dataset_B/scSMD_FL_results_fine_tune_{client}.csv", index=False)
    
df_emissions = pd.DataFrame(emission_results, columns=["Client", "Method", "FineTune", "Emission (kWh)"])
df_emissions.to_csv(f"../../results/dataset_B/scSMD_FL_Client_A_B_emission.csv", index=False)

print("Fine-tuning complete for scSMD (FL) on dataset_B. Results saved to CSV.")
