...
# For federated fine-tuning, use chunk 7 for test and chunks 9 and 10 for clients
...
for client, client_file in clients.items():
    # Prepare client training and test data from CSV
    ...
    for method, params in methods.items():
        ...
        torch.save(model.state_dict(), f"../../models/dataset_C/scCDCG_FL_dataset_C_{method}_without_fine_tune_{client}.pth")
        
        print(f"Training Method: {method}, Client: {client}, With Fine-Tuning")
        tracker = EmissionsTracker()
        tracker.start()
        
        # Initialize a new model and load the pre-trained weights for fine-tuning
        model = scCDCG_scRNAseqClassifier(
            input_size=train_data[0][0].shape[0],
            num_classes=len(torch.unique(torch.tensor([label for _, label in train_data]))),
            dims_encoder=params['dims_encoder'],
        ...
        pre_trained_path = f"../../models/dataset_C/scCDCG_FL_dataset_C_{method}_pre_train.pth"
        ...
        torch.save(model.state_dict(), f"../../models/dataset_C/scCDCG_FL_dataset_C_{method}_with_fine_tune_{client}.pth")
    ...
    df_results.to_csv(f"../../results/dataset_C/scCDCG_FL_results_fine_tune_{client}.csv", index=False)
    
df_emissions = pd.DataFrame(emission_results, columns=["Client", "Method", "FineTune", "Emission (kWh)"])
df_emissions.to_csv(f"../../results/dataset_C/scCDCG_FL_Client_A_B_emission.csv", index=False)
