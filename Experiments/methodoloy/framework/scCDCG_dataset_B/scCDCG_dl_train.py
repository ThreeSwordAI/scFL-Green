...
# Define folder paths for dataset_B (CSV files)
folder_path = "../../processed_data/dataset_B"
# Use chunks 1,2,3,4 for training
training_files = [f"{folder_path}/dataset_B_chunk_{i + 1}.csv" for i in range(4)]
...
# Assume each CSV has 2001 columns: first 2000 are features and the last is the label.
...
for method, params in methods.items():
    ...
    config_path = f"../../models/dataset_B/scCDCG_dataset_B_{method}_config.json"
    with open(config_path, "w") as f:
        json.dump(architecture_config, f)

    # Instantiate the model; input_size is 2000 features.
    ...
    for epoch in range(15):  
        ...
        with tqdm(total=len(train_loader), desc=f"Epoch {epoch + 1}/15", unit="batch") as pbar:
            for inputs, labels in train_loader:
                ...
                optimizer.zero_grad()
                outputs = model(inputs)  # No explicit adjacency is passed.
    ...
    torch.save(model.state_dict(), f"../../models/dataset_B/scCDCG_DL_dataset_B_{method}_pre_train.pth")

per_epoch_results_df = pd.DataFrame(per_epoch_results, columns=['Method', 'Epoch', 'Train_Accuracy', 'Test_Accuracy'])
per_epoch_results_df.to_csv("../../results/dataset_B/scCDCG_DL_per_epoch_result_pre_train.csv", index=False)
...
results_df.to_csv("../../results/dataset_B/scCDCG_DL_results_pre_train.csv", index=False)

print("Pre-training complete for dataset_B. Results saved to CSV.")
