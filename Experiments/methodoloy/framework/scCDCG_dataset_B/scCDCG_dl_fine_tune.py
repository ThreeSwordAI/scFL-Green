...
# Update folder path to dataset_B (CSV files)
data_folder_path = "../../processed_data/dataset_B"
# For fine-tuning, use chunk 7 as test and chunks 9 and 10 as client files
...
# Function to load CSV data for dataset_B:
# Assumes each CSV has 2001 columns: columns 0-1999 are features, column 2000 is the label.
...
def evaluate_model(model, test_loader, device):
    model.eval()
    test_correct, test_total, test_preds, test_labels = 0, 0, [], []
    with torch.no_grad():
...
for client, file in clients.items():
    ...
    for method, params in methods.items():
        ...
        torch.save(model.state_dict(), f"../../models/dataset_B/scCDCG_DL_dataset_B_{method}_without_fine_tune_{client}.pth")
        
        print(f"Training Method: {method}, Client: {client}, With Fine-Tuning")
        tracker = EmissionsTracker()
        tracker.start()
        
        # Load pre-trained weights (using scCDCG naming) for fine-tuning
        model.load_state_dict(torch.load(f"../../models/dataset_B/scCDCG_DL_dataset_B_{method}_pre_train.pth"))
        model = model.to(device)
        ...
        for epoch in range(10):
            ...
            if (train_accuracy >= 0.9) and (accuracy > best_accuracy):
                print(f"Early stopping at epoch {epoch + 1}, best accuracy: {accuracy:.4f}")
                break
        
        emissions = tracker.stop()
        emission_results.append([client, method, "with", emissions])
        torch.save(model.state_dict(), f"../../models/dataset_B/scCDCG_DL_dataset_B_{method}_with_fine_tune_{client}.pth")
    ...
    df_results.to_csv(f"../../results/dataset_B/scCDCG_DL_results_fine_tune_{client}.csv", index=False)
    
    
df_emissions = pd.DataFrame(emission_results, columns=["Client", "Method", "FineTune", "Emission (kWh)"])
df_emissions.to_csv(f"../../results/dataset_B/scCDCG_DL_Client_A_B_emission.csv", index=False)

print("Fine-tuning complete for dataset_B. Results saved.")
