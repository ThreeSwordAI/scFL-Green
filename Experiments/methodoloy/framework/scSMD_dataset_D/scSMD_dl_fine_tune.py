...
# Define file paths for dataset_D (h5ad files)
folder_path = "../../processed_data/dataset_D"
test_file = f"{folder_path}/dataset_D_chunk_11.h5ad"
clients = {
    "A": f"{folder_path}/dataset_D_chunk_9.h5ad",
    "B": f"{folder_path}/dataset_D_chunk_10.h5ad"
}
...
for client, file in clients.items():
    ...
    for method, params in methods.items():
        ...
        # Initialize model from scratch using scSMD (with latent_dim)
        ...
        torch.save(model.state_dict(), f"../../models/dataset_D/scSMD_DL_dataset_D_{method}_without_fine_tune_{client}.pth")
        
        print(f"Training Method: {method}, Client: {client}, With Fine-Tuning")
        tracker = EmissionsTracker()
        tracker.start()
        
        # Load pre-trained FL model weights for scSMD (using same saving style as in scDLC)
        model.load_state_dict(torch.load(f"../../models/dataset_D/scSMD_DL_dataset_D_{method}_pre_train.pth"))
        model = model.to(device)
        ...
        torch.save(model.state_dict(), f"../../models/dataset_D/scSMD_DL_dataset_D_{method}_with_fine_tune_{client}.pth")
    ...
    df_results.to_csv(f"../../results/dataset_D/scSMD_DL_results_fine_tune_{client}.csv", index=False)
    
df_emissions = pd.DataFrame(emission_results, columns=["Client", "Method", "FineTune", "Emission (kWh)"])
df_emissions.to_csv("../../results/dataset_D/scSMD_DL_Client_A_B_emission.csv", index=False)
