...
def load_global_categories(mapping_file="../../processed_data/dataset_E/dataset_E_label_mapping.txt"):
    """
    Loads a mapping of integer keys to label names from a text file.
    Each line in the file should have the format:
      key: value
    Uses a maximum split of 1 to ensure that only the first colon separates the key from the value.
    """
    mapping = {}
...
# Define file paths and client mapping for Dataset E
...
# Create output directories for Dataset E if not present
os.makedirs("../../results/dataset_E", exist_ok=True)
os.makedirs("../../models/dataset_E", exist_ok=True)

def process_client_data(file_path):
    """
    Reads a CSV file where the first 2000 columns are features and the 2001st column is the label.
    Converts features and labels to PyTorch tensors and returns a TensorDataset.
    """
...
def evaluate_model(model, test_loader, device):
    """
    Evaluates the given model on test data and computes accuracy and weighted F1 score.
    """
    model.eval()
...
# Load global categories and determine number of classes (should be 11)
global_categories = load_global_categories()
num_global_classes = len(global_categories)
print("Loaded Global Categories:", global_categories)
print("Num Global Classes is:", num_global_classes)
...
for client, client_file in clients.items():
    ...
    for method, params in methods.items():
        ...
        model = scDLC_scRNAseqClassifier(
            ...
            num_classes=num_global_classes,  # Use 11 global classes
        ...
        torch.save(model.state_dict(), f"../../models/dataset_E/scDLC_FL_dataset_E_{method}_without_fine_tune_{client}.pth")
        ...
        # Initialize a new model and load the pre-trained weights (from training on Dataset E)
        model = scDLC_scRNAseqClassifier(
            input_size=train_data[0][0].shape[0],
            num_classes=num_global_classes,  # Use 11 global classes here as well
            lstm_size=params.get('lstm_size', 64),
            num_layers=params.get('num_layers', 2)
        )
        pre_trained_path = f"../../models/dataset_E/scDLC_FL_dataset_E_{method}_pre_train.pth"
        ...
        torch.save(model.state_dict(), f"../../models/dataset_E/scDLC_FL_dataset_E_{method}_with_fine_tune_{client}.pth")
    ...
    df_results.to_csv(f"../../results/dataset_E/scDLC_FL_results_fine_tune_{client}.csv", index=False)
    
# Save overall emissions results
df_emissions = pd.DataFrame(emission_results, columns=["Client", "Method", "FineTune", "Emission (kWh)"])
df_emissions.to_csv(f"../../results/dataset_E/scDLC_FL_Client_A_B_emission.csv", index=False)
