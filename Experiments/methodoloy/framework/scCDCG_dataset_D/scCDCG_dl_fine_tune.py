...
for client, file in clients.items():
    ...
    for method, params in methods.items():
        ...
        torch.save(model.state_dict(), f"../../models/dataset_D/scCDCG_DL_dataset_D_{method}_without_fine_tune_{client}.pth")
        
        print(f"Training Method: {method}, Client: {client}, With Fine-Tuning")
        tracker = EmissionsTracker()
        tracker.start()
        
        # Load pre-trained weights with new naming for scCDCG
        model.load_state_dict(torch.load(f"../../models/dataset_D/scCDCG_DL_dataset_D_{method}_pre_train.pth"))
        model = model.to(device)
        ...
        torch.save(model.state_dict(), f"../../models/dataset_D/scCDCG_DL_dataset_D_{method}_with_fine_tune_{client}.pth")
    ...
    df_results.to_csv(f"../../results/dataset_D/scCDCG_DL_results_fine_tune_{client}.csv", index=False)
    
    
df_emissions = pd.DataFrame(emission_results, columns=["Client", "Method", "FineTune", "Emission (kWh)"])
df_emissions.to_csv(f"../../results/dataset_D/scCDCG_DL_Client_A_B_emission.csv", index=False)
